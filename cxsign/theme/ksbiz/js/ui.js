...
var ui = {
	...
	initGrid : function(){
		...
		w2ui.grid.on('click', function(event) {
			...
			$('#grid_grid_focus').val(gridList[_idx-1].type + gridList[_idx-1].user_name + gridList[_idx-1].after_date + gridList[_idx-1].org);
	...
	var getDiskListCallback = function(certList){
		...
		if(certList){
			...
			if(certList.certSets.length > 0){
				...
				$('#grid_grid_focus').attr("aria-label", "인증서선택창");
